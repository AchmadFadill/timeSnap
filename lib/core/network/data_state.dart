...
import 'package:timesnap/core/network/base_response.dart';
...
try{
    ...
    if(httpResponse.response.statusCode == HttpStatus.ok){
      ...
      throw DioException(response: httpResponse.response, requestOptions: httpResponse.response.requestOptions);
    }
  } on DioException catch(e){
    try{
      final response = ErrorState.fromJson(jsonDecode(e.response.toString()));
      return ErrorState(message: '${e.response?.statusCode ?? ''} ${response.message}');
    }catch (e1){
